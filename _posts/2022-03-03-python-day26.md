---
layout: single
title: "[Python] 파이썬 챌린지 Day26(U.S. States Game)"
categories: Python
...
- 그동안 for문을 몇줄에 걸쳐서 쓰면서 만들었는데, 이런 새롭고 간편한 방법도 있구나!
...
이번 챌린지를 통하여 배운 부분이나 느낀점은
- 리스트 표현식을 활용해서 예전에 했던 챌린지들을 좀 더 편하게 해결하는 과정을 통해 리스트 표현식의 장점을 뚜렷하게 알 수 있고, 예전에 배운 개념도 한번 더 되짚어 볼 수 있었다.
...
이번 챌린지를 통하여 배운 부분이나 느낀점은
- 솔루션으로 제시된 코드와의 차이점은 나는 파일을 읽어오고 ```splitlines()``` 메서드를 통해 줄바꿈을 기준으로 데이터를 분리해서 리스트로 만들었는데, 솔루션에서는 ```readlines()```로 읽어왔다. 내가 그렇게 한 이유는 리스트 안의 값이 문자열이 아닌 숫자형이어야 한다고 해서 빈칸 표시를 없애야 한다고 생각했는데, 내 생각과는 달리 그저 ```readlines()```로 읽어오고 ```int()```를 씌워주니 공백이 사라지는 것을 알 수 있었다.
...
이번 챌린지를 통하여 배운 부분이나 느낀점은
- 리스트 표현식을 배우고 활용하면서 엄청 편하다고 생각했는데, 사전형은 더 편한 방법인 것 같다.
...
이번 챌린지를 통하여 배운 부분이나 느낀점은
- 아직까지 여러 자료형을 한 챌린지에서 종합적으로 사용하면 그 형식이나 값을 불러올 때 어떻게 해야할지 헷갈린다.
